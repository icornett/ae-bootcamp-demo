...
resource "azurerm_role_assignment" "bootstrap_runner_owner" {
...
resource "time_sleep" "key_vault_rbac_propagation" {
  ...
  depends_on = [
    ...
    azurerm_role_assignment.bootstrap_runner_owner,
...
resource "azurerm_role_assignment" "key_vault_swa" {
  count                = local.swa_principal_id != null ? 1 : 0
  scope                = azurerm_key_vault.main.id
  role_definition_name = "Key Vault Secrets User"
  principal_id         = local.swa_principal_id
}
